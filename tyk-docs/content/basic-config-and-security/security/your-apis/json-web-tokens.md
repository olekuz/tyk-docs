...
You can now configure the clock skew for the three claims we check when validating a JWT `-nbf`, `exp` and `iat` using the following variables. All values are in seconds and are added/subtracted to the current time of the check.  The default is `0` (i.e. no skew). The default is `0` (i.e. no skew).
