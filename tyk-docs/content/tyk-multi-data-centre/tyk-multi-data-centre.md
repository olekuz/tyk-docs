...
With MDCB you can easily have many environments that are logically as well as physically separated. Your control plane (master data centre) will be the management layer for creating all the configurations for all the environments and you can create many logical worker data centres per team and environment (logical separation, so in one physical data centre you can have many). Every worker data centre will have redis and a cluster of gateways.  For example, on your laptop you can run a DC with redis and one gateway, your Dev1 environment (for team1) might have only two gateways, same for Dev2 (for team2) and your production will have it set on auto-scale in all the regions in the world from which you want to serve your users.
