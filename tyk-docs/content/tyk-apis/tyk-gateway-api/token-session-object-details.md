...
* `org_id`: The organisation this user belongs to, this can be used in conjunction with the `org_id` setting in the API Definition object to have tokens "owned" by organisations. See the Organisations Quotas section of the [Tyk Gateway API](/tyk-gateway-api/).
